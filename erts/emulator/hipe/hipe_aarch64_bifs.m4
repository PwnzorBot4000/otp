...
#`define ASM'
include(`hipe/hipe_aarch64_asm.m4')
#`include' "config.h"
...
#  define CALL_BIF(F)	ldr x30, =nbif_impl_##F; str x30, [x0, #P_BIF_CALLEE]; bl hipe_debug_bif_wrapper
#else
#  define CALL_BIF(F)	bl	nbif_impl_##F
#endif'

define(TEST_GOT_MBUF,`ldr x1, [P, #P_MBUF]	/* `TEST_GOT_MBUF' */
	cbz x1, #8
	bl nbif_$1_gc_after_bif')
...
$1:
	...
	SAVE_CONTEXT_BIF
	str	x1, [x0, #P_ARG0]	/* Store BIF__ARGS in def_arg_reg[] */
	add	x1, x0, #P_ARG0
	CALL_BIF($2)
	TEST_GOT_MBUF(1)

	/* Restore registers. Check for exception. */
	cmp	x0, #THE_NON_VALUE
	RESTORE_CONTEXT_BIF
